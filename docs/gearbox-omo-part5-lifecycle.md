...
| # | 缺失 | 影响 | 建议 |
|---|------|------|------|
| 1 | 单一销毁端口 | 每个路径独立销毁，可能泄漏 | 引入 `destroy_task()` 统一入口 |
| 2 | 启动 reconciliation | 崩溃重启后残留 `running` 记录不会处理 | 启动时扫描 `.gearbox-agent/workers/*/task-record.json` |
| 3 | LRU eviction | 无 resident 上限控制 | 新增 `admit_resident()` 和 LRU |
| 4 | Evictable 状态排除 cancelled | `cancelled` 不应被自动驱逐 | 当前 completed_archive 已处理 |
| 5 | TTL 清理 | 无时间基的记录过期 | 启动时删除超过 N 天的旧记录 |
| 6 | `lost` 记录保护 | lost 记录在 pid 确认死前不删除 | Gear 当前没有 lost 状态 |
