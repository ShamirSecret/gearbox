...
当前已经完成到 **Milestone 2 的原生 GUI 接入、事件流和取消阶段**，并已进入 **Goal Pursuit Runtime**。下一步正在推进 **Milestone 3：TypeScript 应用生成**。

已完成：

- `crates/gearbox_agent` 本地 runtime/CLI 原型已经建立。
- `gear run "<prompt>" --workspace <path>` 可以生成 `.gearbox-agent/` 状态目录、goal/task/event ledger 和 artifacts。
- 已实现 TypeScript/Python/Rust 语言 profile 检测、shell verification、diff/scope 检查、opencode worker packet、verification artifact、final report。
- Gearbox GUI 里已经区分两个原生 agent：
  - `Agent`：保留原 Zed native agent 行为。
  - `Gear`：Gearbox 专用原生 agent。
- GUI 的 `Gear` 会话已经接入 `gearbox_agent::runtime::Orchestrator`，不再只是复用 Zed native agent 控制核心。
- Gear runtime events 已经转换为 ACP/native thread message streaming，Agent Panel 可以看到 Gear 阶段性状态。
- GUI cancel 会触发 Gear session 的 cancellation token，orchestrator/worker/verification 会在阶段边界和 shell 命令执行时停止。
- `gearbox_agent` 已引入 `WorkerAdapter` / `WorkerRegistry` 边界，runtime 可选择 `opencode` / `codex` / `claude` / `zed_agent` / `custom` worker kind。
- `gearbox_agent` runtime 已经是有限轮次 goal pursuit loop：每轮 worker 后执行 diff、verification、goal review，并在失败时创建 repair iteration。
- TypeScript Web App 的空 workspace prompt 已开始进入默认 TypeScript profile，spec/plan 会写出默认栈和 build/README 要求。
- Gear GUI 会话已经把当前 native thread 选中的 provider/model 作为 `coordinator_model` 传入 `gearbox_agent`，并写入 goal ledger、worker packet、spec/plan/final report。
- Gear GUI 会在启动 runtime 前用选中的 provider/model 生成第一版 `coordinator_brief`，runtime 将它作为 planning/review context 传给 worker 和交付报告；测试 fake provider 会跳过真实模型调用。
- Gear GUI 对短问候和 small-talk 做入口分流，不再对 `你好` 这类输入启动 goal pursuit loop 或写入 `.gearbox-agent/` 产物。

当前未完成：

- TypeScript 应用生成还没有完成 3 个样例需求的端到端验收。
- TypeScript worker 还没有完成真实 scaffold 的 3 个样例稳定验收。
- `coordinator_brief` 目前是单次前置 brief；还没有做成每轮 goal review 后都调用 provider 重新判断下一步。
- 高级 worker registry 仍是轻量边界，还没有形成可动态注册/成本路由的 worker catalog。
- ACP stdio External Agent server 还没有实现；当前路线先走 Gearbox GUI 原生集成。

下一步：

1. 把单次 `coordinator_brief` 扩展为每轮 review/repair 前的 provider-backed coordinator hook，并支持取消。
2. 固化 TypeScript Web App 默认栈、worker packet 输入和 build 验证路径。
3. 跑通 3 个 TypeScript Web App 样例的端到端生成验收。
4. 根据样例失败结果继续增强 scaffold/repair prompt。
5. 如果后续需要非 Gearbox GUI 宿主，再补 ACP stdio External Agent server。

## Goal Pursuit Runtime

Gear 的 runtime 不应只是 “一次调用 worker，然后验证并结束”。它应该是一个追目标的控制层：
...
goal
  ...
  -> run verification
  -> review whether the goal is satisfied
  -> repair or re-plan when incomplete
  -> repeat until complete, blocked, or limited
```

### Gear 的职责

- 持有 goal、验收标准、预算、轮次、事件流和 artifact ledger。
- 自己做计划、审查、验证解释和下一步决策。
- 把代码实现交给 worker，但不把 goal ownership 交给 worker。
- 在每轮结束后问自己：目标是否达到？还差什么？失败是否重复？下一轮应该修复、缩小范围、重新计划，还是停止并请求用户？

### Worker 的职责

- 接收 Gear 给出的 bounded packet。
- 完成一轮局部代码实现或修复。
- 输出可审查的 stdout/stderr/result artifact。
- 不决定最终 goal 是否完成。

### MVP 限制

- 第一版只接一个 worker adapter：当前 `opencode` 命令。
- 第一版使用有限轮次和有限 repair 次数，而不是无限自治循环。
- 第一版的 goal-check 由 Gear runtime 根据 verification、scope check、worker 状态和重复失败信号决定。
- 当前已抽出第一版 `WorkerAdapter` / `WorkerRegistry`，所有 worker kind 先走同一个 command adapter；后续再接 Codex、Claude、Zed Agent 或 ACP worker 的原生协议。
...
```json
{
  ...
  "coordinator_brief": "模型生成的短计划和审查提示",
  "summary": ""
}
```
