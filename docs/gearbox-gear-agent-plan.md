...
当前已经完成到 **Milestone 2 的原生 GUI 接入、事件流和取消阶段**，下一步正在进入 **Goal Pursuit Runtime**。
...
- 当前 runtime 仍偏一次性 pipeline，还没有形成持续追目标的 plan/code/review/repair/goal-check 循环。
- 验证失败后目前会创建 repair task，但还没有自动派发 focused repair worker、复查目标状态并继续下一轮。
- ACP stdio External Agent server 还没有实现；当前路线先走 Gearbox GUI 原生集成。

下一步：

1. 将 `gearbox_agent` runtime 改为有限轮次的 goal pursuit loop。
2. 每一轮执行 `plan -> code(worker) -> review -> repair/next-plan -> goal-check`。
3. 再进入 Milestone 3：TypeScript Web App 样例生成验收。
4. 如果后续需要非 Gearbox GUI 宿主，再补 ACP stdio External Agent server。
