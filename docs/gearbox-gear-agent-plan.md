...
- `gearbox_agent` 已开始引入 `WorkerAdapter` / `WorkerRegistry` 边界，runtime 不再直接绑定 `OpencodeWorker` 类型。
...
- 当前已抽出第一版 `WorkerAdapter` / `WorkerRegistry`，但 registry 里仍只有 `opencode` adapter；后续再接 Codex、Claude、Zed Agent 或 ACP worker。
