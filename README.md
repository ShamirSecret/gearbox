...
Gearbox 不是从零开始重写编辑器，而是在成熟编辑器核心之上做产品化改造：核心能力尽量跟随上游更新，Gearbox 自己的品牌、汉化、设置资源和用户体验改动尽量隔离在独立层中。
...
- 希望在上游快速迭代的基础上维护一个独立中文品牌版本。
- 希望以后可以继续做产品级封装，而不是每次上游更新都手工重做大量改动。
...
Gearbox 的汉化不是一次性机械替换，而是分层推进：

- 资源层：默认设置、快捷键说明、打包元数据、README。
- 入口层：欢迎页、菜单栏、命令入口、最近项目、文件查找。
- 设置层：设置页标题、分组、设置项说明、Provider 配置。
- Agent 层：Agent 面板、AI Provider、模型选择、权限和沙箱说明。
- 深层提示：编辑器错误、确认弹窗、调试器和语言服务器提示。
