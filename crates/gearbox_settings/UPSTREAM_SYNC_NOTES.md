...
- `crates/agent_ui/src/conversation_view/thread_view.rs` now also consumes `TaskManager::snapshot()` directly for a Gear-only native activity-bar section: task counts, recent task/attempt summaries, current worker output, and direct open buttons for task/attempt `result` / `outcome` artifacts.
- That Gear-only activity-bar section now also exposes `View Output`, `Interrupt`, and `Cancel Task` controls backed by `open_markdown_in_workspace()`, `interrupt_gear_task()`, and `cancel_gear_task()`.
- The same Gear-only section now adds first-pass panel behavior: `All` / `Active` / `Attention` filters, running/pending-first sorting, and `Show More` / `Show Less` when more than six tasks match.
...
| `[NEW]` | `generate_gear_coordinator_review()` | Async LLM review call after each Gear iteration. Prompt includes task id, worker kind/category, route reason, worker outcome, commands, failures, outcome path, verification, scope, and diff. |
...
| `[NEW]` | `NativeAgentConnection::cancel_gear_task()` | Gear-only bridge from GUI-native observer controls to `TaskManagerControl::cancel_current_task()` |
...
| `[MOD]` | `cancel()` | +Gear TaskManagerControl cancel for the current worker, +Gear token cancel; still calls `thread.cancel()` for native thread cleanup |
...
- `thread_view.rs`: Gear-only native TaskManager observer section in `render_activity_bar()`. It uses `NativeAgentConnection::gear_task_manager_snapshot()` and adds task/attempt summary rows plus task/attempt artifact open buttons without changing upstream `Agent` / external session rendering.
- `thread_view.rs`: that Gear-only observer section also adds explicit `Interrupt`, `Cancel Task`, and full-output open actions for the current worker.
- `thread_view.rs`: that same section now keeps its own Gear-only filter/show-more view state and reorders task rows so `running`/`pending` tasks surface first.
...
- Gear runtime Phase 3 follow-up: connect model availability policy to the GUI/provider registry instead of only CLI/env unavailable-model lists
