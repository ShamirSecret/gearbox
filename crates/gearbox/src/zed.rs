...
pub fn init(cx: &mut App) {
    ...
    // When Gearbox logs to stdout rather than the log file, avoid registering
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut App) -> WindowOptions {
    ...
    WindowOptions {
        ...
        // On macOS, Gearbox handles window movement itself, so disable AppKit's titlebar dragging.
...
fn show_software_emulation_warning_if_needed(
        ...
        && !gearbox_env_set("GEARBOX_ALLOW_EMULATED_GPU", "ZED_ALLOW_EMULATED_GPU")
    {
        ...
        let message = format!(
            db::indoc! {r#"
            Gearbox uses {} for rendering and requires a compatible GPU.
    ...
    impl AboutWindow {
        fn new(cx: &mut Context<Self>) -> Self {
            ...
            let message: SharedString =
                format!("Gearbox {release_channel_name} {version} {debug}").into();
