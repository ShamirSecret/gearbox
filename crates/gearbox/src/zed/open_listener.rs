...
async fn open_local_workspace(
    ...
    // `std::env::current_dir()`, since the Gearbox app process's cwd is typically
...
mod tests {
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Gearbox to add it to the current workspace
        open_workspace_file(
            path!("/root/file6.txt"),
            workspace::OpenOptions {
        ...
        // but tell Gearbox to NOT add it to the current workspace
        open_workspace_file(
            path!("/root/file7.txt"),
            workspace::OpenOptions {
                workspace_matching: workspace::WorkspaceMatching::None,
    ...
    /// the Gearbox-side `handle_cli_connection` on the GPUI foreground executor,
