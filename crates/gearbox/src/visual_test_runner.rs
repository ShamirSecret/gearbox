...
//! Visual Test Runner
//!
//! This binary runs visual regression tests for Gearbox's UI. It captures screenshots
//! of real Gearbox windows and compares them against baseline images.
//!
...
mod constants {
    ...
    /// Embedded test image (Gearbox app icon) for visual tests.
...
fn run_visual_tests(project_path: PathBuf, update_baseline: bool) -> Result<()> {
    ...
    // Test settings use Courier font, but we want the real Gearbox fonts for visual tests
    cx.update(|cx| {
        settings::init(cx);
    });
    ...
    // Initialize all Gearbox subsystems
    cx.update(|cx| {
        gpui_tokio::init(cx);
...
This is a test project for visual testing of Gearbox.
...
fn run_agent_thread_view_test(
    ...
    let send_future = thread.update(cx, |thread, cx| {
        thread.send(vec!["Show me the Gearbox logo".into()], cx)
    });
