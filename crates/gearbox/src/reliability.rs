    ...
    for (index, gpu) in metadata.gpus.iter().cloned().enumerate() {
        ...
        form = form
            .text(
                ...
                "A GPU found on the users system. May or may not be the GPU Gearbox is running on",
            )
...
// users - except - we know internal users use Gearbox.
