...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Gearbox failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.gearbox.Oops";
            proxy
                .add_notification(
                    notification_id,
...
fn main() {
    ...
    // args are appended verbatim and would otherwise be misinterpreted as Gearbox's
    // own arguments.
    ...
    // Keep Gearbox user data separate from the upstream editor application.
    if let Some(dir) = &args.user_data_dir {
        paths::set_custom_data_dir(dir);
    } else {
        let data_dir = paths::home_dir()
            .join(".local")
            .join("share")
            .join("gearbox");
    ...
    // `gearbox --askpass` makes Gearbox operate in nc/netcat mode for use with askpass.
    ...
    // `gearbox --crash-handler` makes Gearbox operate in minidump crash handler mode.
    ...
    if args.system_specs {
        ...
        println!("Gearbox System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let crash_handler = if should_install_crash_handler {
        Some(
            app.background_executor().spawn(crashes::init(
                ...
                |pid| paths::temp_dir().join(format!("gearbox-crash-handler-{pid}")),
    ...
    app.run(move |cx| {
        ...
        settings::set_settings_asset_loader(gearbox_settings::load);
        settings::init(cx);
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Gearbox would exit silently.
...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Gearbox`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zed`.
    /// On Windows, the default is `%LOCALAPPDATA%\Gearbox`.
    #[arg(long, value_name = "DIR", verbatim_doc_comment)]
    user_data_dir: Option<String>,

    /// The username and WSL distribution to use when opening paths. If not specified,
    /// Gearbox will attempt to open the paths directly.
    ///
    ...
    /// prevents Gearbox from starting, so you can't run `gearbox: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having Gearbox run a separate
    ...
    /// by having Gearbox act like netcat communicating over a Unix socket.
    #[arg(long)]
    #[cfg(not(target_os = "windows"))]
    #[arg(hide = true)]
    ...
    /// The PID of the Gearbox process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    ...
    /// Unix socket path for IPC with the parent Gearbox process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
