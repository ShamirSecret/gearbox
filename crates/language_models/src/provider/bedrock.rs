    ...
    fn settings_view(&self, _cx: &mut App) -> Option<ProviderSettingsView> {
        ...
        Some(ProviderSettingsView::SubPage(
            ...
            .description(InlineDescription::Text(
                            "To use Gearbox Agent with Bedrock, set a custom authentication strategy in your settings or use static credentials. Mantle-only models (e.g. GPT-5.5, GPT-5.4, Grok 4.3) additionally require IAM permissions for the `bedrock-mantle` endpoint.".into(),
                        )),
        ))
    }
}
